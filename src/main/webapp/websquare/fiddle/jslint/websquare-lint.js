...
---------------
  ...
  Usage:
    websquare_lint(html_source);
*/

function websquare_lint(html_source, lint_options) {
//Wrapper function to invoke all the necessary constructors and deal with the output.
      ...
      if (this.last_token === 'TK_TAG_SCRIPT') { //check if we need to format javascript
        ...
        if (typeof temp_token !== 'string') {
          return temp_token;
   	    ...
   	    var leftTrimRegExp =  new RegExp("^\\s\\s*");
	    	...
	    	temp_token = temp_token.replace(leftTrimRegExp, '').replace(rightTrimRegExp, '');
				var cdata = false;
				if( temp_token.substring(0,9) == "<![CDATA[" && temp_token.substring(temp_token.length-3,temp_token.length) == "]]>" ) {
					cdata =true;
					temp_token = temp_token.substring(9, temp_token.length-3 );
				}
        ...
        token = js_beautify(temp_token, {
          ...
          'brace_style': this.brace_style
				...
				if (!JSLINT(token, lint_options)) {
					this.lint_output.push(JSLINT.report(true));
					this.lint_JS.push(token);
				} else {
					this.lint_output.push("JS code is valid");
					this.lint_JS.push(token);
				}

        if( cdata ) {
        	token = "<![CDATA[" + token.replace(leftTrimRegExp, '').replace(rightTrimRegExp, '') + "]]>";
        }
  ...
  return {"js": multi_parser.lint_JS, "lint" : multi_parser.lint_output};
}
